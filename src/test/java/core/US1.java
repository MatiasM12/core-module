...
public class US1 {
	...
	public void escenario() throws ClassNotFoundException, InstantiationException, IllegalAccessException, NoSuchMethodException, InvocationTargetException, IOException {
		...
		String[] validArgs = new String[] { url, us };
		Observable obv = new Core().init(validArgs);
	...
	@Test
	void CA1() {
		Map<String, Boolean> newTest = createMap("CA1", true); // [CA1 : TRUE]
		TestSummary newTSWithOneResult = createTS(newTest);
		assertEquals(((TSStub) ts.update(newTSWithOneResult)).sizeCA(), 1);
	}

	@Test
	void CA2() throws ClassNotFoundException, InstantiationException, IllegalAccessException, NoSuchMethodException {
		Map<String, Boolean> emptyTest = new HashMap<String, Boolean>(); // []
		TestSummary newEmptyTS = createTS(emptyTest);
		assertEquals(((TSStub) ts.update(newEmptyTS)).sizeCA(), 0);
	}
